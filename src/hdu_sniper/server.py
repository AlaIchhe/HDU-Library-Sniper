"""FastAPI/ASGI 宿主：应用实例由入口注入，不依赖组合根。"""
...
from hdu_sniper.ui.flet_view import flet_main, resolve_assets_dir


def create_server_app(application: SniperAppProtocol) -> FastAPI:
    """构建 FastAPI 应用，并将给定门面实例注入所有路由和 Flet Web 宿主。"""
    app = FastAPI(
    ...
    def _require_authenticated() -> SniperAppProtocol:
    ...
    def status() -> dict:
        current = _require_authenticated()
        plans = current.list_plans()
        return {
            "state": current.state,
    ...
    def list_bookings() -> dict:
        """返回当前图书馆账户的预约记录。"""
        return {"bookings": _require_authenticated().list_bookings()}

    def _booking_query(operation, booking_id: str) -> dict:
        current = _require_authenticated()
        try:
            return {"response": operation(current, booking_id)}
    ...
    def get_booking_status(booking_id: str) -> dict:
        ...
        return _booking_query(
            lambda current, value: current.get_booking_status(value),
            booking_id,
        )
    ...
    def get_latest_comeback_time(booking_id: str) -> dict:
        ...
        return _booking_query(
            lambda current, value: current.get_latest_comeback_time(value),
            booking_id,
        )
    ...
    def run_booking(execute_at: str | None = None) -> dict:
        ...
        current = _require_authenticated()
        try:
            results = current.run_booking(execute_at=execute_at)
        except ValueError as exc:
            raise HTTPException(
    ...
    def _booking_action(operation, booking_id: str) -> dict[str, str | bool]:
        current = _require_authenticated()
        try:
            success, message = operation(current, booking_id)
        except ValueError as exc:
            raise HTTPException(
                status_code=http_status.HTTP_404_NOT_FOUND,
                detail=str(exc),
    ...
    def cancel_remote_booking(booking_id: str) -> dict[str, str | bool]:
        ...
        return _booking_action(
            lambda current, value: current.cancel_remote_booking(value),
            booking_id,
        )
    ...
    def check_in_booking(booking_id: str) -> dict[str, str | bool]:
        """签到一条待签到预约。"""
        return _booking_action(
            lambda current, value: current.check_in_booking(value),
            booking_id,
        )
    ...
    def come_back_booking(booking_id: str) -> dict[str, str | bool]:
        """让暂离中的预约恢复为使用中。"""
        return _booking_action(
            lambda current, value: current.come_back_booking(value),
            booking_id,
        )
    ...
    def renew_booking(booking_id: str) -> dict[str, str | bool]:
        """将暂离中的预约续回为使用中。"""
        return _booking_action(
            lambda current, value: current.renew_booking(value),
            booking_id,
        )
    ...
    def test_check_in(booking_id: str) -> dict[str, str | bool]:
        ...
        current = _require_authenticated()
        try:
            success, message = current.test_check_in(booking_id)
        except ValueError as exc:
            raise HTTPException(
                status_code=http_status.HTTP_404_NOT_FOUND,
                detail=str(exc),
    ...
    def auto_check_in() -> dict:
        """自动签到所有已经进入签到窗口的预约。"""
        return {"results": _require_authenticated().auto_check_in()}

    @app.get("/api/v1/auto-check-in", tags=["auto-check-in"])
    def auto_check_in_status() -> dict:
        """返回自动签到的开关、协议同意状态与当前协议版本。"""
        return _require_authenticated().auto_check_in_status()
    ...
    def enable_auto_check_in() -> dict[str, str | bool]:
        ...
        current = _require_authenticated()
        success, message = current.enable_auto_check_in()
        if not success:
            raise HTTPException(
                status_code=http_status.HTTP_409_CONFLICT,
                detail=message,
    ...
    def disable_auto_check_in() -> dict[str, str | bool]:
        """关闭自动签到并移除相关系统任务。"""
        current = _require_authenticated()
        success, message = current.disable_auto_check_in()
        if not success:
            raise HTTPException(
                status_code=http_status.HTTP_409_CONFLICT,
                detail=message,
    ...
    def leave_booking(booking_id: str) -> dict[str, str | bool]:
        """让使用中的预约暂离。"""
        return _booking_action(
            lambda current, value: current.leave_booking(value),
            booking_id,
        )
    ...
    def sign_out_booking(booking_id: str) -> dict[str, str | bool]:
        """签退一条使用中的预约。"""
        return _booking_action(
            lambda current, value: current.sign_out_booking(value),
            booking_id,
        )
    ...
    def list_schedules() -> dict:
        ...
        current = _require_authenticated()
        try:
            tasks = current.list_scheduled_tasks()
    ...
    def run_schedule(task_name: str) -> dict[str, str | bool]:
        """请求系统任务计划程序立即运行一个本应用任务。"""
        current = _require_authenticated()
        try:
            success, message = current.run_scheduled_task(task_name)
        except ValueError as exc:
            raise HTTPException(
                status_code=http_status.HTTP_404_NOT_FOUND,
                detail=str(exc),
    ...
    def delete_schedule(task_name: str) -> dict[str, str | bool]:
        ...
        current = _require_authenticated()
        try:
            success, message = current.delete_scheduled_task(task_name)
        except ValueError as exc:
            raise HTTPException(
                status_code=http_status.HTTP_404_NOT_FOUND,
                detail=str(exc),
