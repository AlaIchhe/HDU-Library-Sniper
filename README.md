...
5. （可选）进入“调度”页，阅读并同意风险协议后启用“自动签到”；系统会在
   签到窗口开启时自动签到，并在电脑登录后兜底检查一次。
...
不是。自动签到默认关闭，属于可选功能。启用前必须阅读并勾选风险协议；
该功能可能违反学校或图书馆平台规则，存在账号封禁风险，后果由用户自行承担。

## 注意事项
* **建议下载便携版，因为其他版本我没有测试过**
* 请合理设置重试间隔，遵守图书馆相关规定。
* 学号、密码和 Cookie 属于敏感信息，请勿分享或提交到代码仓库。
* 自动签到为自愿启用的可选功能，默认关闭；启用即表示已知悉封号等风险，
  本工具不承担由此产生的任何后果。
* 本项目仅供学习和个人使用。
