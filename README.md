...
认证成功后，认证页面会退出主导航，只显示“方案”和“执行”。如需主动更新凭据，可点击页头的“重新认证”；远端登录态失效时，程序会自动返回认证页面。
...
- 点击"立即尝试预约后天"可以手动验证方案。

手动执行仍然只能预约后天，不提供目标日期或等待时间设置。
...
“执行”工作区仅展示该固定任务的只读健康状态，并提供“检查并修复”；不会向用户开放执行时间配置。
