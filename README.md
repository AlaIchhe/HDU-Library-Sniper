...
- 把**脚本所在目录**作为工作目录（不再硬编码路径）
- 自动从系统 PATH 查找 `python`（或读取 `PYTHON_EXE` 环境变量）
- 注册一个每日 `19:59` 触发的计划任务（NT AUTHORITY\SYSTEM，电池供电也可运行）
- 输出任务名、工作目录、日志目录、实际 Python 路径，便于二次确认
- 找不到 Python 会**立即报错（退出码 7）**，避免静默失败后过几天才发现没抢到
...
| 环境变量 | 作用 | 缺省值 |
| --- | --- | --- |
| `PYTHON_EXE` | 自定义 python.exe 绝对路径 | 自动从 PATH 查找 `python3`/`python` |
| `SNIPER_WORKDIR` | 自定义工作目录 | **脚本所在目录** |
| `SNIPER_DAILY_AT` | 每日触发时间 | `07:59PM` |
| `SNIPER_TASK_NAME` | 计划任务名称 | `HDU-Library-Sniper-Daily` |
| `SNIPER_LOG_HISTORY` | 保留的归档日志数量 | `30` |
...
- 默认输出到 `<工作目录>/logs/task_log.txt`
- 单日志超过 5MB 自动滚动为 `task_log_<时间戳>.txt`，保留最近 30 个
- 写盘前会捕获 Python 异常退出码（非 0 时打 ⚠️ 标记）
...
2. 名称：`HDU 抢座`，触发器：每日 `19:59`
3. 操作 → 启动程序：
   - **程序**：`powershell.exe`
   - **参数**：`-NoProfile -WindowStyle Hidden -File "<本项目绝对路径>\AutoSchedule.ps1" -Execute`
